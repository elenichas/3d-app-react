...
class DimensionDistance extends Dimension {
  constructor(container) {
    super();
    this.container = container;
    console.log("container", this.container);
    this.createGizmo = function (container) {
      this.dimensionGizmo = new DimensionGizmoDistance(this, container);
    ...
    this.getInfo = function () {
      ...
      var controlPoints = this.dimensionGizmo.getControlPointsWorld();
      var info = [];
...
class DimensionGizmoDistance extends DimensionGizmo {
  constructor(dimension, container) {
    ...
    this.handleGizmos = {
      TOPLINE: [
        new Mesh(
          new CylinderGeometry(1, 1, 1, 4, 1, false),
          new DimensionGizmoMaterial({
            color: 0xff0000,
            opacity: 0.4,
          })
        ),
      ...
      STARTLINE: [
        new Mesh(
          new CylinderGeometry(0.5, 0.5, 1, 4, 1, false),
          new DimensionGizmoMaterial({
            color: 0x000000,
            opacity: 0.1,
          })
      ...
      START: [
        new Mesh(
          ...
          new DimensionGizmoMaterial({ color: 0xff0000, opacity: 0.4 })
        ),
      ],
      END: [
        new Mesh(
      ...
      STARTARROW: [
        new Mesh(
          new CylinderGeometry(3, 0, 6, 8, 1, false),
          new DimensionGizmoMaterial({
            color: 0xff0000,
            opacity: 0.4,
          })
    ...
    this.pickerGizmos = {
      TOPLINE: [
        new Mesh(
          new CylinderGeometry(3, 3, 1, 4, 1, false),
          new DimensionGizmoMaterial({ visible: false })
        ),
        new Vector3(0, 0.5, 0),
        new Vector3(Math.PI / 2, 0, 0),
      ],
  ...
  addControlPoint(point, object, forceAdd, face, callbackAddedObject) {
    var object =
      this.controlPoints.length < 2 ? object : this.controlPoints[1].object; //for 3rd point use object of the 2nd point
    super.addControlPoint(point, object, forceAdd, face, callbackAddedObject);
    if (this.controlPoints.length == 1) {
      //1st point - add dimension to object
      if (object) {
        object.add(this.dimension);
        if (callbackAddedObject) callbackAddedObject(this.dimension);
      }
      ...
      if (forceAdd !== true) {
        var point = new Vector3()
          .copy(controlPoints[0])
          .add(controlPoints[2])
          .sub(controlPoints[1]);
        super.addControlPoint(
          point,
          this.controlPoints[0].object,
          false,
          face,
          callbackAddedObject
        ); //for 4th point use object of the 1st point
      }
    }
    this.show();
  }
    ...
    switch (scope.controlPoints.length) {
      case 4:
        ...
        if (distance > 2 * width) {
          ...
          verticalCorrection = new Vector3()
            .copy(controlPoints[3])
            .sub(controlPoints[0])
            .setLength(width * 2.0);
      ...
      case 2:
        ...
        horisontalCorrection = new Vector3()
          .copy(horisontal)
          .setLength(width * 6.0);
