...
//DimensionGizmoMaterial
class DimensionGizmoMaterial extends MeshBasicMaterial {
...
export { DimensionGizmoMaterial };

//Measurements
//Dimension interface (base class)
class Dimension extends Object3D {
  constructor() {
    super();
    this.dimensionGizmo = null;
  ...
  constructor(dimension, container) {
    ...
    this.init = function () {
      ...
      if (this.container) {
        ...
        this.text.setStyle("font", [
          "normal normal normal 13px/normal 'Helvetica Neue', arial, sans-serif",
        ]);
        this.text.setStyle("overflow", ["hidden"]);
        this.container.appendChild(this.text.dom);
      ...
      for (var i in this.handleGizmos) {
        ...
        this.transformGizmo(
          handle,
          this.handleGizmos[i][1],
          this.handleGizmos[i][2]
        );
      ...
      for (var i in this.pickerGizmos) {
        ...
        this.transformGizmo(
          picker,
          this.pickerGizmos[i][1],
          this.pickerGizmos[i][2]
        );
    ...
    this.show = function () {
      for (var j in this.handles.children)
        this.handles.children[j].visible = false;
      for (var j in this.pickers.children)
        this.pickers.children[j].visible = showPickers;
      if (this.text && this.text.dom.textContent) this.text.setDisplay("block");
      this.dimension.visible = true;
    };

    this.isVisible = function () {
      return this.dimension.visible;
    };

    this.getTextPicker = function () {
      if (this.pickerGizmos["TEXT"]) return this.pickerGizmos["TEXT"][0];
      else return null;
    };
    ...
    this.dragMove = function (gizmo, eye, cameraPosition) {
      ...
      if (gizmo == "TOPLINE") {
        var intersection = this.linePlaneIntersection(
          cameraPosition,
          eye,
          this.dragOrigin,
          this.dragNormal
        ...
        if (intersection) {
          this.onGizmoMoved(
            gizmo,
            this.lastPosition.sub(intersection).negate()
          );
          this.lastPosition = new Vector3().copy(intersection);
        }
      } else {
        console.log("can't move it from arrows");
      }
    };
    ...
    this.getCenterPointWorld = function () {
      ...
      if (controlPoints.length > 0) center.divideScalar(controlPoints.length);
    ...
    this.getScreenCoords = function (position, camera) {
      ...
      if (vector.project) vector.project(camera);
      else projector.projectVector(vector, camera);
      return new Vector2(
    ...
    this.offsetControlPoint = function (i, offset) {
      //get points in world coordinates
      if (i >= this.controlPoints.length || !this.controlPoints[i].point)
        return;
      if (this.controlPoints[i].object) {
    ...
    this.clean = function () {
      if (this.container && this.text) {
        this.text.setDisplay("none");
        this.container.removeChild(this.text.dom);
      }
    };

    this.restore = function () {
      if (this.container && this.text) {
        if (this.text.dom.textContent) this.text.setDisplay("block");
        this.container.appendChild(this.text.dom);
      }
    };

    this.removeUIObject = function () {
      if (this.dimension && this.dimension.parent)
        this.dimension.parent.remove(this.dimension);
    };
    ...
    this.linePlaneIntersection = function (
      lineOrigin,
      lineNormal,
      planeOrigin,
      planeNormal
